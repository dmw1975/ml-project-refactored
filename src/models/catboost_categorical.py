"""
Redirect to enhanced CatBoost categorical implementation.
"""

import sys
from pathlib import Path
sys.path.append(str(Path(__file__).parent.parent.parent))
from scripts.archive.enhanced_catboost_categorical import train_enhanced_catboost_categorical
...
def train_catboost_categorical(X, y, dataset_name, categorical_columns, test_size=0.2, random_state=42):
    ...
    """
    return train_enhanced_catboost_categorical(X, y, dataset_name, categorical_columns, test_size, random_state)
...
def train_catboost_categorical_models(datasets=['all']):
    """
    Train CatBoost models with categorical features on specified datasets.
    This function provides compatibility with the existing pipeline.
    """
    print("🌳 Training Enhanced CatBoost models with native categorical features...")
    ...
    for dataset_name, (X, description) in dataset_configs.items():
        ...
        try:
            ...
            print(f"Error training CatBoost on {dataset_name}: {e}")
            import traceback
            traceback.print_exc()
    
    # Save combined results
    if all_results:
        combined_path = settings.MODEL_DIR / 'catboost_models.pkl'
        with open(combined_path, 'wb') as f:
            pickle.dump(all_results, f)
        print(f"\n✅ All CatBoost models saved to: {combined_path}")
    
    print(f"\n✅ CatBoost categorical training completed! {len(all_results)} models trained.")
    return all_results


if __name__ == "__main__":
    results = train_catboost_categorical_models()