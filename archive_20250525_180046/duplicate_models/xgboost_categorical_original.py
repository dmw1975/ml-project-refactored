"""XGBoost models with native categorical feature support."""
...
def train_xgboost_with_categorical(X, y, dataset_name, categorical_features=None):
    """
    Train XGBoost model with native categorical feature support.
    ...
    """
    print(f"\n--- Training XGBoost with Categorical Support on {dataset_name} ---")
    ...
    # Handle missing values and prepare categorical features for XGBoost
    X_clean = X.copy()
    ...
    # Enable categorical features for XGBoost (requires XGBoost >= 1.6.0)
    X_clean[cat_features_present] = X_clean[cat_features_present].astype('category')
    
    # Split the data
    X_train, X_test, y_train, y_test = train_test_split(
        X_clean, y, test_size=0.2, random_state=42, 
        stratify=X_clean[cat_features_present[0]] if cat_features_present else None
    )
    
    # XGBoost parameters optimized for categorical features
    params = {
        'objective': 'reg:squarederror',
        'eval_metric': 'rmse',
        'tree_method': 'hist',  # Required for categorical features
        'enable_categorical': True,  # Enable native categorical support
        'max_cat_to_onehot': 4,  # Use native categorical for features with >4 categories
        'max_depth': 6,
        'learning_rate': 0.1,
        'subsample': 0.8,
    ...
    # Map feature names back if needed
    if len(feature_importance) != len(X_clean.columns):
        # XGBoost uses f0, f1, f2... for feature names
        feature_mapping = {f'f{i}': col for i, col in enumerate(X_clean.columns)}
        feature_importance['feature'] = feature_importance['feature'].map(feature_mapping).fillna(feature_importance['feature'])
...
def train_xgboost_categorical_models(datasets=['all']):
    ...
    """
    print("🌳 Training XGBoost models with native categorical features...")
    ...
    for dataset_name, (X, description) in dataset_configs.items():
        ...
        try:
            result = train_xgboost_with_categorical(X, target, dataset_name)
            results[f'XGBoost_{dataset_name}_categorical'] = result
            
            # Save model
            model_filename = f'xgboost_{dataset_name.lower()}_categorical.pkl'
            model_path = io.save_model(result['model'], model_filename, settings.MODEL_DIR)
            print(f"Model saved to: {model_path}")
            
            # Save feature importance
            importance_path = settings.FEATURE_IMPORTANCE_DIR / f'XGBoost_{dataset_name}_categorical_importance.csv'
    ...
    print(f"\n✅ XGBoost categorical training completed! {len(results)} models trained.")
    return results


if __name__ == "__main__":
    # Test the implementation
    results = train_xgboost_categorical_models()
    print("XGBoost categorical models training completed!")