...
def plot_statistical_tests_filtered(tests_df=None, allowed_model_types=None):
    ...
    # Set up output directory - use performance directory instead of statistical_tests
    perf_dir = settings.VISUALIZATION_DIR / "performance"
    io.ensure_dir(perf_dir)
    
    # Use performance directory directly
    output_dir = perf_dir
    ...
    datasets = set()
    # Dictionary to map dataset codes to display names with 'R' for random
    dataset_display_names = {
        'LR_Base': 'LR Base',
        'LR_Yeo': 'LR Yeo',
        'LR_Base_Random': 'LR Base R',  # Use 'R' to indicate random variable
        'LR_Yeo_Random': 'LR Yeo R'     # Use 'R' to indicate random variable
    }
    
    for model_name in allowed_models:
        if 'LR_Base_Random' in model_name:
            datasets.add('LR_Base_Random')
        elif 'LR_Yeo_Random' in model_name:
            datasets.add('LR_Yeo_Random')
        elif 'LR_Base' in model_name:
    ...
    for dataset in datasets:
        ...
        # Add legend to explain the white border and 'R' notation
        plt.figtext(0.5, 0.01, 
                  "White border indicates significance (p < adjusted α after Holm-Bonferroni correction)\n" +
                  "R indicates dataset containing random variable", 
        ...
        for i in range(n_models):
            for j in range(n_models):
                if not np.isnan(p_matrix[i, j]):
                    p_val = p_matrix[i, j]
                    # Remove the 'p' to get additional space
        ...
        # Use display name with R for random datasets
        display_name = dataset_display_names.get(dataset, dataset)
        ax.set_title(f'Statistical Significance - {display_name}', fontsize=16, pad=20)
